...
export type Preferences = { game:'all'|'TCG'|'VGC'; kinds:EventKind[]; district:string; theme:'light'|'dark' };
...
export function readPreferences(raw:string|null,systemDark=false):Preferences {
 const defaults:Preferences={game:'all',kinds:[],district:'all',theme:systemDark?'dark':'light'};
 ...
 return {game,kinds:kindsForGame(game,Array.isArray(value.kinds)?value.kinds:[]),district:typeof value.district==='string'&&value.district.trim()?normalizePlace(value.district):'all',theme:['light','dark'].includes(value.theme)?value.theme:defaults.theme};
