...
export function matchesDistrict(place:string, selection:string|readonly string[]) { return typeof selection==='string' ? selection==='all'||place===selection : selection.length===0||selection.includes(place); }
export function filterEvents(events:Tournament[], game:string, kinds:string[], district:string|readonly string[], today:string, includeFriendlies=false) { return events.filter(e=>e.date>=today && (game==='all'||e.game===game) && (e.kind==='friendly'?includeFriendlies:(kinds.length===0||kinds.includes(e.kind))) && matchesDistrict(e.district,district)); }
