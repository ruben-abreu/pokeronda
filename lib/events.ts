...
export function dateParts(date:string) { const d=new Date(date+'T12:00:00Z');return {day:date.slice(8),month:new Intl.DateTimeFormat('pt-PT',{month:'short',timeZone:'UTC'}).format(d).replace('.',''),weekday:new Intl.DateTimeFormat('pt-PT',{weekday:'short',timeZone:'UTC'}).format(d).replace('.','')}; }
